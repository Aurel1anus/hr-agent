...
def seed(db: Session):
    if db.scalar(select(func.count()).select_from(Job)):
        return
    j1 = Job(
        title="HRBP 实习生",
        department="人力资源部",
    ...
    for idx, (name, school, year, _, stage, blocked, title, hours) in enumerate(specs):
        ...
        db.flush()
        a = Application(
            job_id=j1.id, candidate_id=c.id, stage=stage, blocked_by=blocked
        )
        db.add(a)
        db.flush()
        t = Task(
            application_id=a.id,
            title=title,
            due_at=datetime.now() + timedelta(hours=hours),
            priority=Priority.HIGH if hours < 0 else Priority.NORMAL,
        )
        db.add(t)
    ...
    interview = Interview(
        ...
        round_name="一面",
        interviewer_name="李然",
        scheduled_start_at=datetime.now().replace(
            hour=14, minute=0, second=0, microsecond=0
        ),
        scheduled_end_at=datetime.now().replace(
    ...
    db.add(interview)
    db.flush()
    first.current_interview_id = interview.id
    db.commit()
